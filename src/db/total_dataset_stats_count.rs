...
pub fn get_dataset_stats_count(conn: &Connection) -> DatasetStatsCount {
    let mut stmt = conn
        .prepare("SELECT * FROM total_dataset_stats_count")
        .unwrap();
...
fn calculate_dataset_stats_count(conn: &Connection) -> DatasetStatsCount {
    ...
    let mut splits_count_stmt = conn
        .prepare("SELECT COUNT(*) FROM dataset_stats where dataset_stats.status_code = 200")
        .unwrap();
    let splits_count: i64 = splits_count_stmt.query_row([], |row| row.get(0)).unwrap();

    let mut configs_count_stmt = conn
        .prepare(
            "
    SELECT COUNT(*) FROM 
    ...
    let mut datasets_count_stmt = conn
        .prepare(
            "
    SELECT COUNT(*) FROM 
